...
REQUEST_LIST = "!REQUEST_LIST" + str(" " * (64 - len("!REQUEST_LIST")))
...
def handle_client(conn, addr):
    ...
    while connected:
        ...
        if msg_length:  # If !msg_lenth.equals(null)
            ...
            with lock:  # Check for existing client and add if new
                if not any(client[1] == addr for client in active_clients):
            ...
            if msg == DISCONNECT_MESSAGE:
                connected = False
                with lock:  # Remove client on disconnect
                    active_clients[:] = [client for client in active_clients if client[1] != addr]
                print(f"[Closing Connection] {addr}")
            elif msg == REQUEST_LIST:
